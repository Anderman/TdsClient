    ...
    internal class MultipartIdentifier
    {
        ...
        */
        ...
        private static void IncrementStringCount(string name, string[] ary, ref int position, string property)
        {
            ...
            if (position >= limit) throw ADP.InvalidMultipartNameToManyParts(property, name, limit);
        ...
        {
            if (limit <= 0) throw ADP.InvalidMultipartNameToManyParts(property, name, limit);

            if (-1 != leftQuote.IndexOf(separator) || -1 != rightQuote.IndexOf(separator) || leftQuote.Length != rightQuote.Length) throw ADP.InvalidMultipartNameIncorrectUsageOfQuotes(property, name);

            var parsedNames = new string[limit]; // return string array                     
            var stringCount = 0; // index of current string in the buffer
            var state = MPIState.MPI_Value; // Initialize the starting state

            var sb = new StringBuilder(name.Length); // String buffer to hold the string being currently built, init the string builder so it will never be resized
                ...
                switch (state)
                {
                    case MPIState.MPI_Value:
                    {
                        ...
                        else if (-1 != (quoteIndex = leftQuote.IndexOf(testchar)))
                        {
                            ...
                            sb.Length = 0;
                            if (!removequotes) sb.Append(testchar);
                    ...
                    case MPIState.MPI_ParseNonQuote:
                    {
                        ...
                        else if (IsWhitespace(testchar))
                        {
                            // If it is Whitespace 
                            parsedNames[stringCount] = sb.ToString(); // Set the currently parsed string
                            if (null == whitespaceSB) whitespaceSB = new StringBuilder();
            ...
            {
                if (ThrowOnEmptyMultipartName) throw ADP.InvalidMultipartName(property, name); // Name is entirely made up of whitespace
            }
            else
            {
                // Shuffle the parsed name, from left justification to right justification, i.e. [a][b][null][null] goes to [null][null][a][b]
                var offset = limit - stringCount - 1;
                if (offset > 0)
                    for (var x = limit - 1; x >= offset; --x)
                    {
