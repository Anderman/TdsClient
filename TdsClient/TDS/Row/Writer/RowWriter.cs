    ...
    public class RowWriter
    {
        ...
        private static Action<TdsColumnWriter, T> _GetWriter<T>(Mapping[] mapping)
        {
            ...
            foreach (var map in mapping)
            {
                ...
                {
                    throw new Exception($"Could not create a mapping between property {map.PropertyName}:{property.PropertyType.Name} and column index:{map.SqlIndex}:{writeValue.Type}", ex);
                }
            }
