...
namespace Medella.TdsClient.TDS.Row.Reader
{
    public class RowReader
    {
        ...
        private static MethodCallExpression GetValue(Expression reader, int columnIndex, int tdsType) => Expression.Call(reader, SqlTypes.ContainsKey(tdsType) ? SqlTypes[tdsType] : throw new Exception($"TdsType not supported:{tdsType} index:{columnIndex}"), Expression.Constant(columnIndex));
